...
Use TodoWrite to create a checklist of all tasks. This is your single source of truth for progress. Update it after each task completes. **TodoWrite state does not survive compaction -- for a long plan, also mirror each clean task into a durable append-only ledger file under `.aegis-scratch/` (see `subagent-execution/per-task-execution-stages.md`) so a compacted controller never re-dispatches a finished task.**
...
See `subagent-execution/per-task-execution-stages.md` for the full per-stage instructions (exact reviewer prompts, FAIL-path mechanics, max-cycle escalation) and the model-selection table (mechanical → haiku/sonnet, integration → sonnet, architecture → opus; reviewers always sonnet+).
...
- **Hand large artifacts as files, not pasted bodies; pin explicit SHAs.** Full briefs, diffs, and implementer reports travel as file PATHS under `.aegis-scratch/` (see `rules/scratch-dir-convention.md`), not pasted inline -- pasted text stays controller-resident and is re-read every turn. Never embed a moving ref (`HEAD~1`/`HEAD`) in a dispatch prompt; resolve and pin the literal SHA. Full technique: `subagent-execution/per-task-execution-stages.md`.
...
`brainstorm-spec → implementation-planner → orchestrate → finishing-branch`, and the
