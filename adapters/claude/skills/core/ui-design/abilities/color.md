...
**Pair with:** `style-families.md` (defines mood), `design-systems.md` (consumes the output as tokens), `ux-reasoning.md` (contrast rules §18).

## When to use

- You have a named style (from `style-families.md`) but no colour values yet.
...
Take the brand colour or pick a hue that matches the style mood (see `style-families.md` palette mood). Note:
...
See `color/palette-reference.md` for the style-family palette table, the seven anti-patterns to avoid, the canonical output-table format consumed by `design-systems.md`, and the final review checklist.

## See also

- `style-families.md` — choose the visual style first; it constrains the palette mood and background treatment.
- `design-systems.md` — consumes this palette output as design tokens.
- the `ui-design` parent skill — pillar 3 (colour) verifies the finished system against these ratios.
- `ux-reasoning.md` — rules §6 (visual hierarchy via colour), §18 (focus ring contrast), §25 (single-source tokens).
