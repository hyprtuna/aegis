...
1. **Open the rule catalog.** Read `ux-reasoning/rule-catalog.md` — each rule has a title, rationale, concrete example, and common violation. The 25 rules are grouped into 5 sections (Perception, State, Interaction, Motion, Content).
2. **Audit a target screen.** Walk each rule against the screen. Record PASS / DRIFT / N/A using the template in `ux-reasoning/audit-checklist.md`.
...
See `ux-reasoning/rule-catalog.md` for the full text of every rule.
...
- the `ui-design` parent skill — 6-pillar audit that this checklist feeds into.
- `style-families.md`, `color.md`, `typography.md` — construction phase; resolve before running this checklist.
- `design-systems.md` — token step; rule §25 (single-source tokens) verifies the system.
