...
> **Invoke via `Skill({skill: "aegis:research"})`.** This is a skill, not an agent. If you reached for the Agent tool, you're using the wrong primitive.
...
<!-- One block per option (2-4 options). -->

### Option {{ slot.option.label }}: {{ slot.option.name }}

**Pros:** {{ slot.option.pros }}

**Cons:** {{ slot.option.cons }}

## Recommendation

{{ slot.recommendation }}

**Key trade-off:** {{ slot.keyTradeoff }}
