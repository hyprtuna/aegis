...
- **Skill discovery:** Plugin `config(cfg)` pushes one absolute path per skill bucket into `cfg.skills.paths`. The bucket list is derived from the filesystem at boot (any directory under `skills/` holding at least one `<name>/SKILL.md`) rather than enumerated in the plugin, so dissolving or adding a bucket needs no edit — and cannot leave a deleted directory registered. Today that resolves to `skills/core` alone; the plugin warns loudly if it resolves to nothing.
