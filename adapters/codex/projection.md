...
| `pre-tool-use-deny` | partial | Projected to Codex `PreToolUse`. Bundled at `.codex/plugins/aegis/hooks/pre-tool-use-deny.sh`. Deny config (`permissions.json`) bundled alongside — script resolves it via `$(dirname "$0")/permissions.json` (script-relative, fail-open). Codex deny contract is Claude-compatible for `permissionDecision: "deny"` (verified 2026-06-20), so the destructive-ops guard projects intact. **Gap — `permissionDecision: "ask"`:** the protected-branch push check emits `ask`, which is a documented *Claude* PreToolUse value; Codex is **not verified** to honour it, and the failure mode is unknown (ignored, or the whole decision object discarded — which would take the destructive `deny` down with it). Treated as unverified, not assumed working. Not solved here: the Codex hook binding is being dropped this release, which retires the question rather than answering it. Destructive-ops runtime fire test also pending interactive Codex. |
