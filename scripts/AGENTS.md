...
- `test-deny-hook.mjs` — regression test for the PreToolUse deny hook (`.claude-plugin/hooks/pre-tool-use-deny.sh`); 57 deny/allow cases + 3 documented known-gaps. Includes compound-command coverage: each `git push` in a command is scanned against its OWN refspecs, so a downstream protected-branch mention is not misread as the destination, and a later push in the same command is still caught. Requires bash + jq.
