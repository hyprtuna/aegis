...
| `FRONTMATTER` | Every canonical surface (`skills/**/SKILL.md`, `agents/*.md`, `commands/*.md`, `rules/*.md`, `hooks/*.md`) opens with a closed frontmatter block carrying the lean 4 keys: `name`, `description`, `visibility`, `platforms` — **and does not carry the retired `kind:` key** (no host recognised it, the projector already dropped it, and the directory already states the kind). | error | Add or close the frontmatter and include the missing key; delete any `kind:` line. |
...
| `HOOK_INTENT` | Every `hooks/*.json` intent satisfies the canonical contract: schema shape (kind/name/description/visibility/platforms), event→dispatch support table, `.md` pairing when required, command-file existence, compaction pre⇔post symmetry, plugin.json drift (D6 — regenerates the expected Claude hooks block in-memory and compares to committed `.claude-plugin/plugin.json`), x-claude binding completeness when `platforms ⊇ claude`, x-opencode binding completeness when `platforms ⊇ opencode`, and per-host adapter gap-coverage (every shipped intent must have a hook-matrix row in every `adapters/<host>/projection.md`). Plus four checks listed separately below: rejected `enabled` field, OpenCode key contract, Claude hook-script orphans, and rejected hook platforms. Note the rule is **hand-rolled** — it loads no schema file (Aegis ships zero dependencies, so nothing can execute JSON Schema); `manifest/schemas/hook-intent.schema.json` documents the contract for humans and enforces nothing. | error (all checks hard-fail) | Fix the intent JSON, add the missing .md sibling, re-run `node scripts/project.mjs` to regenerate the Claude hooks block, or add the missing hook-matrix row in the failing adapter's `projection.md`. |
| `HOOK_INTENT` — `enabled` field rejected | A hook intent carrying an `enabled` field is rejected outright (`scripts/validate/hook-intent.mjs`). There is no parked/disabled state: a hook either ships (its intent projects) or is deleted from `hooks/` entirely. A disabled-but-present intent is a file that reads as shipped and behaves as absent. | error | Delete the `enabled` field to ship the hook, or delete the whole intent to retire it. |
| `HOOK_INTENT` — OpenCode key contract | `x-opencode.event` must be one of the **flat dotted** hook keys declared on `@opencode-ai/plugin`'s `Hooks` interface (`experimental.chat.messages.transform`, `experimental.session.compacting`), and **no two intents may bind the same key**. A nested object binding declares a different property, so the handler registers and is never invoked, silently. The generated handler object is a JS object literal, so a duplicate key silently wins and the loser never fires. | error | Use the literal dotted key string from the type contract; give each intent its own key. |
| `HOOK_INTENT` — Claude hook-script orphans | Every file in `.claude-plugin/hooks/` must be in the tree's **keep-set**: referenced by some intent's `x-claude.command`, or declared in some intent's `x-claude.helpers`. A script orphaned by a deleted intent still ships inside the plugin and is reachable by anything discovering the directory by convention, while nothing in canonical mentions it. `projectHooks()` prunes these on every projector run, printing each deleted path and raising rather than descending into a directory; this rule is the tripwire for a projector that was never re-run. The keep-set comes from one exported function, `hookTreeKeepSet()` in `scripts/lib/hook-projection.mjs`, which this rule and the projector's prune both call — so the prune can never delete a file this rule demands. | error | Run `node scripts/project.mjs` to prune the orphan, add the intent that binds it, or declare it in the sourcing hook's `x-claude.helpers`. |
| `HOOK_INTENT` — helper declarations | `x-claude.helpers` must be an array of **bare filenames** (the tree is flat), and every declared helper must exist under `.claude-plugin/hooks/`. A declaration is what protects a file from the prune, so a dangling entry reserves a name no file answers to. This replaced an earlier `_`-prefix naming convention — a file is now protected because an intent claims it, never because of how it is spelled. | error | Create the declared helper, correct the filename, or drop the declaration. |
| `HOOK_INTENT` — flat command paths | `x-claude.command` must match `^\.claude-plugin/hooks/[^/]+$` — the hook tree is flat, no subdirectories. The prune walks the directory one level deep and refuses to descend, so a nested path would name a script it cannot account for. | error | Move the script to the flat directory and repoint the binding. |
| `HOOK_INTENT` — rejected hook platforms | A hook intent whose `platforms` includes `codex` is rejected. Codex's `plugin_hooks` feature is removed upstream, so a plugin-shipped hook cannot fire there and Aegis has no Codex hook projector. `codex` stays in the recognised platform enum specifically so it draws this explicit message instead of a generic unknown-platform error — accepting it silently would validate an intent that projects nothing, which is a silent failure rather than a gap. The projector's `loadHookIntents()` throws on the same condition, so a projector-only workflow cannot slip past. | error | Drop `codex` from the intent's `platforms`; see `adapters/codex/projection.md`. |
