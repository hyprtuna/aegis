...
| `HOOK_INTENT` — Claude hook-script orphans | Every file in `.claude-plugin/hooks/` must be referenced by some intent's `x-claude.command`. A script orphaned by a deleted intent still ships inside the plugin and is reachable by anything discovering the directory by convention, while nothing in canonical mentions it. `projectHooks()` prunes these on every projector run; this rule is the tripwire for a projector that was never re-run. | error | Run `node scripts/project.mjs` to prune the orphan, or add the intent that binds it. |
