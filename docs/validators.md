...
| `HOOK_INTENT` — Claude hook-script orphans | Every file in `.claude-plugin/hooks/` must be referenced by some intent's `x-claude.command`, except `_`-prefixed shared helpers (exempt here and in the projector's prune — the two must agree). A script orphaned by a deleted intent still ships inside the plugin and is reachable by anything discovering the directory by convention, while nothing in canonical mentions it. `projectHooks()` prunes these on every projector run, printing each deleted path and raising rather than descending into a directory; this rule is the tripwire for a projector that was never re-run. | error | Run `node scripts/project.mjs` to prune the orphan, add the intent that binds it, or rename a shared helper to `_<name>.sh`. |
| `HOOK_INTENT` — flat command paths | `x-claude.command` must match `^\.claude-plugin/hooks/[^/]+$` and `x-codex.command` must match `^\./hooks/[^/]+$` — both hook trees are flat, no subdirectories. Each prune walks its directory one level deep and refuses to descend, so a nested path would name a script it cannot account for; on the Codex side the path is additionally emitted verbatim into `hooks.json` while the script is written to its basename, so nesting ships a manifest pointing at a file that was never bundled. | error | Move the script to the flat directory and repoint the binding. |
| `HOOK_INTENT` — codex needs a Claude command binding | An intent whose `platforms` includes `codex` must also carry an `x-claude` binding with `dispatch: "command"`. `projectCodexHooks()` copies the script it bundles **exclusively** from `x-claude.command`; a codex-only intent emits a `hooks.json` entry naming a script that never gets bundled — a binding that cannot fire (Iron Law 6). `x-codex.dispatch` is likewise restricted to `command`, the only value with a projector path (others emit `{"type":"command"}` with the command key dropped). | error | Add the `x-claude` command binding, or drop `codex` from `platforms`. |
