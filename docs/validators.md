...
Every validator rule introduced in **v0.0.6** lands **warn-only**. It surfaces a
warning, the build stays green, and canonical content gets one release to clear
before the rule graduates to a hard-fail in **v0.0.7**. This applies to the
...
`validate-prose.mjs` gate. Existing pre-v0.0.6 rules already hard-fail.
(The body-length check shipped as `SKILL_BODY_LONG` in v0.0.6; in v0.0.14 it
was renamed and moved to the dedicated `SKILL_SIZE` rule — see *New v0.0.14
rules* below.)

Note: `SKILL_OVER_CODEX_CAP` was removed in v0.2.0. The per-body
...
These shipped before v0.0.6. Unless noted, they push to `errors` (hard-fail).
...
## New v0.0.6 rules (warn-only)

These rules all push to `warnings` only in v0.0.6 and graduate to hard-fail in
v0.0.7.

Note: `SKILL_OVER_CODEX_CAP` (the per-body 8 KB cap rule, `skill-codex-cap.mjs`)
was removed in v0.2.0 — see the *Warn-then-error rollout* note above.
...
**Single owner, no double-finding.** As of v0.0.14 this rule is the
...
exceeded 100 lines when v0.0.14 opened; hard-failing immediately would break the
build. Progressive disclosure (v0.0.14 Phase A) restructures them a few at a
time into a lean `SKILL.md` plus `references/`/`abilities/` overflow — the
v0.0.14 release split the four largest (`sdd-workflow`, `two-stage-review`,
...
cleared, following the v0.0.6 → v0.0.7 warn→error graduation convention.
...
body-length cap moved out of this module to `skill-size.mjs` in v0.0.14 — see
`SKILL_SIZE` above.)
...
**Stage:** **warn-only** in v0.0.9. It surfaces the remaining unwired-producer
backlog (producers get wired, or given a `// REASON:`, in later releases). It
graduates to **hard-fail** in a later release, consistent with the v0.0.6 →
v0.0.7 warn → error convention.

**Graduation precondition (do NOT skip):** the v0.0.9 heuristic keys on the
...
Three rules land in v0.0.12: a warn-only trigger-phrase lint, a doc-drift
...
**Stage:** **warn-only** in v0.0.12, mirroring the v0.0.6 → v0.0.7 warn → error
cadence. It **graduates to hard-fail in v0.0.13**. The lint is purely diagnostic
this release — no canonical descriptions are rewritten in v0.0.12; the backlog it
...
- **Oversize skills — NOT owned here.** The audit's "oversize skills" item is
  ...
  owner of the >100-line body warning (v0.0.14 renamed it from the former
...
**Stage:** **warn-only** in v0.0.12 (new-rule cadence).
...
the v0.0.7 graduation and not used in CI yet.
