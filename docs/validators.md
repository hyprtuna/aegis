...
| `HOOK_INTENT` — flat command path | `x-claude.command` must match `^\.claude-plugin/hooks/[^/]+$` — the hooks tree is flat, no subdirectories. The prune walks that directory one level deep and refuses to descend, so a nested path would name a script it cannot account for. | error | Move the script to `.claude-plugin/hooks/<name>.sh` and repoint `x-claude.command`. |
