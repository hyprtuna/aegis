...
2. **v0.0.1 scope: Canonical port + working Claude plugin.** Full Anvil parity for portable content; a `.claude-plugin/plugin.json` + `marketplace.json` so users can install Aegis as a Claude Code plugin from local path.
...
5. **Frontmatter: Lean 5-field.** `kind, name, description, visibility, platforms`. `source` added only on migrated items. Adapter-specific metadata uses `x-<adapter>` extension namespace.
...
7. **Hooks & statusline: Defer to v0.0.6** except a single Claude `SessionStart` bootstrap hook in v0.0.1.
8. **HTML output: 3 templates v0.0.1, full coverage by v0.1.0.** `implementation-plan.html`, `code-review.html`, `status-report.html`. Coverage tracked in `.aegis/index/html-templates.md`.
...
| Hooks | `hooks/<event>.md` (intent) + `.claude-plugin/hooks/...` (impl) | Defer; v0.0.1 ships SessionStart for Claude only. |
...
| Host | First Projection | Status v0.0.1 | Notes |
|---|---|---|---|
| Claude Code | Native plugin | **Shipped** | `.claude-plugin/plugin.json` + marketplace.json + SessionStart hook |
| OpenCode | Native plugin | Documented gap | v0.0.3 |
| Codex | Skills-first | Documented gap | v0.0.4 — skills + AGENTS.md + MCP |
| Cursor | Rules-only | Documented gap | v0.0.5 — `.cursor/rules/*.mdc` + MCP |
| Zed | Rules + ACP host | Documented gap | v0.0.5 — `.rules` + AGENTS.md + MCP |

## Release Architecture

| Release | Outcome |
|---|---|
| v0.0.1 | Canonical Anvil port + Claude Code projection working. Full Anvil parity for portable content. |
| v0.0.2 | OpenCode projection (native agents/commands/skills/plugin). |
| v0.0.3 | Codex projection (skills-first). |
| v0.0.4 | Cursor + Zed projections (rules-first). |
| v0.0.5 | Portable hook intents + adapter hook implementations beyond SessionStart. |
| v0.0.6 | Statusline schema + host adapters; portable guardrails; transcript-based validation. |
| v0.1.0 | All html-effectiveness pattern families covered; coverage gate enforced. |
