...
| Hooks | `hooks/hooks.json` (plugin-bundled) or `[hooks]` in config | V | `[features] hooks` stable, on by default. Events: SessionStart, SubagentStart, PreToolUse, PermissionRequest, PostToolUse, PreCompact, PostCompact, UserPromptSubmit, SubagentStop, Stop. |
