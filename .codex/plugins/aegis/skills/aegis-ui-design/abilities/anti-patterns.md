...
**Announce:** I'm using the `ui-design` skill's `anti-patterns.md` ability to flag UI anti-patterns in this edit.
...
See `anti-patterns/rule-catalog.md` for the full 15-rule catalog. Each rule has an anti-pattern, a "why", and a "do instead" remediation:
...
When flagging a violation, cite the rule slug (e.g., `low-contrast-text`) and the file:line. Open `anti-patterns/rule-catalog.md` for the full "why" + "do instead" before recommending a fix.
