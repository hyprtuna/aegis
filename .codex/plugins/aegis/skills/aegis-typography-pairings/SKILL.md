---
name: aegis-typography-pairings
...
**Pair with:** `aegis:style-selection` (constrains mood), `aegis:color-palette-design` (contrast ratios), `aegis:design-system-generation` (emits tokens).
...
1. **What style family?** (From `aegis:style-selection`.) Each family has a preferred typography mood.
...
See `abilities/industry-pairings.md` for the 11 industry-specific display/body pairings (SaaS, Fintech, Editorial, Wellness, E-commerce ×2, Healthcare, Gaming/Crypto, EdTech, Agencies), each with mood notes, body-size baselines, and Google Fonts links.
...
- `aegis:style-selection` — style constrains typography mood; resolve style first.
- `aegis:color-palette-design` — pair with the chosen contrast ratios to verify text legibility.
- `aegis:design-system-generation` — emits tokens + component defaults from this output.
- `aegis:ux-reasoning-rules` — rule §16 (touch target sizing affects font-size decisions on mobile).
