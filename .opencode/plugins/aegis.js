...
export const AegisPlugin = async () => {
  ...
  return {
    ...
    /**
     * Inject the Aegis bootstrap into the first user message of each session.
     * Guarded by `<!-- aegis:bootstrap -->` to prevent double-injection.
     */
    experimental: {
      // OpenCode compaction bridge (AG-0010 D8). The handler body lives in the
      // generated AEGIS:HOOKS-GEN region above; this reference is hand-maintained.
      session: { compacting: aegisCompaction },
      chat: {
        messages: {
          transform: async (_input, output) => {
